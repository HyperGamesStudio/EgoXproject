...
namespace Egomotion.EgoXproject.Internal
{
    internal class XcodeProjectManipulator
    {
        ...
        public bool ApplyChanges(XcodeChangeFile changes)
        {
            ...
            if (!ApplyAppConfigChanges(changes.AppConfigEnabled, changes.AppConfig))
            {
                return false;
            }
            ...
            if (!ApplyManualEntitlementsChanges(changes.ManualEntitlements))
            {
                return false;
            }
        ...
        bool ApplyManualEntitlementsChanges(PListDictionary changes, bool forceReplace = false) {
            ApplyEntitlementsChanges(changes,true);
            return true;
        }
        
        bool ApplyAppConfigChanges(PListArray enabled, PListDictionary changes, bool forceReplace = false) {
            PListDictionary plistChanges = new PListDictionary();
            ...
            try
            {
                foreach (var systemCapability in active)
                {
                    ...
                    case SystemCapability.AppSandbox:
                        ApplyAppSandboxCapability(capability as AppSandboxCapability);
                        break;
                    ...
                    case SystemCapability.AppleArcade:
                        ApplyAppleArcadeCapability(capability as AppleArcadeCapability);
                        break;                    
        ...
        void ApplyICloudCapability(ICloudCapability capability)
        {
            ...
            {
                entitlementChanges.Add("com.apple.developer.ubiquity-kvstore-identifier", !string.IsNullOrEmpty(capability.UbiquityKvstoreIdentifier) ? capability.UbiquityKvstoreIdentifier : "$(TeamIdentifierPrefix)$(CFBundleIdentifier)" );
            }
        ...
        //iCloud
        void ApplyAppSandboxCapability(AppSandboxCapability capability)
        {

            var entitlementChanges = new PListDictionary();
            entitlementChanges.Add("com.apple.security.app-sandbox", true);
            
            if(capability.KeyNetworkServer) entitlementChanges.Add(AppSandboxCapability.KEY_NETWORK_SERVER,true);
            if(capability.KeyNetworkClient) entitlementChanges.Add(AppSandboxCapability.KEY_NETWORK_CLIENT,true);
            
            if(capability.KeyCamera) entitlementChanges.Add(AppSandboxCapability.KEY_CAMERA,true);
            if(capability.KeyAudio) entitlementChanges.Add(AppSandboxCapability.KEY_AUDIO,true);
            if(capability.KeyUSB) entitlementChanges.Add(AppSandboxCapability.KEY_USB,true);
            if(capability.KeyPrinting) entitlementChanges.Add(AppSandboxCapability.KEY_PRINTING,true);
            if(capability.KeyBluetooth) entitlementChanges.Add(AppSandboxCapability.KEY_BLUETOOTH,true);
            
            if(capability.KeyContacts) entitlementChanges.Add(AppSandboxCapability.KEY_CONTACTS,true);
            if(capability.KeyLocation) entitlementChanges.Add(AppSandboxCapability.KEY_LOCATION,true);
            if(capability.KeyCalendar) entitlementChanges.Add(AppSandboxCapability.KEY_CALENDAR,true);
            
            if(capability.KeyUserSelectedFiles == AppSandboxCapability.READ_WRITE_OPTIONS.ReadOnly) entitlementChanges.Add(AppSandboxCapability.KEY_USER_SELECTED_FILES + AppSandboxCapability.EXT_READ_ONLY,true);
            else if(capability.KeyUserSelectedFiles == AppSandboxCapability.READ_WRITE_OPTIONS.ReadWrite) entitlementChanges.Add(AppSandboxCapability.KEY_USER_SELECTED_FILES + AppSandboxCapability.EXT_READ_WRITE,true);
            
            if(capability.KeyDownloadsFolder == AppSandboxCapability.READ_WRITE_OPTIONS.ReadOnly) entitlementChanges.Add(AppSandboxCapability.KEY_DOWNLOADS_FOLDER + AppSandboxCapability.EXT_READ_ONLY,true);
            else if(capability.KeyDownloadsFolder == AppSandboxCapability.READ_WRITE_OPTIONS.ReadWrite) entitlementChanges.Add(AppSandboxCapability.KEY_DOWNLOADS_FOLDER + AppSandboxCapability.EXT_READ_WRITE,true);
            
            if(capability.KeyPicturesFolder == AppSandboxCapability.READ_WRITE_OPTIONS.ReadOnly) entitlementChanges.Add(AppSandboxCapability.KEY_PICTURES_FOLDER + AppSandboxCapability.EXT_READ_ONLY,true);
            else if(capability.KeyPicturesFolder == AppSandboxCapability.READ_WRITE_OPTIONS.ReadWrite) entitlementChanges.Add(AppSandboxCapability.KEY_PICTURES_FOLDER + AppSandboxCapability.EXT_READ_WRITE,true);
            
            if(capability.KeyMusicFolder == AppSandboxCapability.READ_WRITE_OPTIONS.ReadOnly) entitlementChanges.Add(AppSandboxCapability.KEY_MUSIC_FOLDER + AppSandboxCapability.EXT_READ_ONLY,true);
            else if(capability.KeyMusicFolder == AppSandboxCapability.READ_WRITE_OPTIONS.ReadWrite) entitlementChanges.Add(AppSandboxCapability.KEY_MUSIC_FOLDER + AppSandboxCapability.EXT_READ_WRITE,true);
            
            if(capability.KeyMoviesFolder == AppSandboxCapability.READ_WRITE_OPTIONS.ReadOnly) entitlementChanges.Add(AppSandboxCapability.KEY_MOVIES_FOLDER + AppSandboxCapability.EXT_READ_ONLY,true);
            else if(capability.KeyMoviesFolder == AppSandboxCapability.READ_WRITE_OPTIONS.ReadWrite) entitlementChanges.Add(AppSandboxCapability.KEY_MOVIES_FOLDER + AppSandboxCapability.EXT_READ_WRITE,true);
        ...
        {
            //_pbxproj.EnableSystemCapability("com.apple.developer.arcade-operations", true);
            //update pbxproject
            //_pbxproj.EnableSystemCapability("com.apple.developer.arcade-operations", true);
